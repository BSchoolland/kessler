...
function updatePlayer(ctx: Ctx, input: InputFrame): void {
  ...
  if (!grounded && p.attackBuffer > 0 && !p.swing) {
    if (s.gunCd > 0) {
      ...
      const pr: Projectile = { id: s.nextId++, pos: add(p.pos, scale(facingV, p.radius + 6)), vel: add(scale(facingV, GUN.speed), scale(p.vel, 0.3)), radius: GUN.radius, life: GUN.life, damage: GUN.damage, hue: 190, friendly: true, knockback: GUN.knockback * mods.knockbackMult, slug: true, seek: 0, bomb: false };
